...
# counterfactual economy where pennies and nickels have been eliminated. The
# simulation is conducted for the paper "Cash Payments and the Penny Policy
# Debate" by Brian Prescott and Oz Shy.
...
symmetric_rounding <- function(amount) {
  # We have to make this adjustment to the rounding so that values ending with
  # a factor of $0.05 will round up rather than down.
  flag <- as.logical(
    stringr::str_sub(stringi::stri_reverse(amount), 1, 1) == "5" &
    stringr::str_sub(stringi::stri_reverse(amount), 1, 2) != "5." &
    as.character(amount) != "5"
  )
  adj_amount <- ifelse(
    test = flag == TRUE,
    yes = 0.01,
  ...
  using_amount <- amount + adj_amount
  counterfactual_amount <- round(using_amount / 0.10) * 0.10
...
# the nearest 10 cents or they will leave their prices alone.
asymmetric_rounding <- function(amount) {
  # This is an extension of the conditional written in the symmetric function.
  flags <- 5:1 %>%
    as.character() %>%
    as.list() %>%
    purrr::map(
      ...
      .f = function(n) {
        output <- as.logical(
          stringr::str_sub(stringi::stri_reverse(amount), 1, 1) == n &
          stringr::str_sub(stringi::stri_reverse(amount), 1, 2) !=
            stringr::str_c(n, ".") &
          as.character(amount) != n &
          amount != floor(amount)
        )
  ...
  names(flags) <- stringr::str_c(
    c("five", "four", "three", "two", "one"), "cents_flag"
  )

  if (sum(unlist(flags)) > 0) {
    adj_position <- which(flags == TRUE)
    adj_amount <- adj_position / 100
    using_amount <- amount + adj_amount
  ...
  counter_amount <- round(x = using_amount, digits = 1)
...
counterfactual_sim <- function(rounding_policy) {
  ...
  if (initial_run == TRUE) {
      if (run_stylized == TRUE) {
        stylized_exercise_3 <- calc_optimal_tokens(
          amounts_list = as.list(seq(4, 5, 0.10)),
          using_tokens = c(
            0.10, 0.25, 0.50, 1, 5, 10, 20, 50, 100
          ),
          only_20note = FALSE
        )
    ...
    optimal_tokens_baseline <- counterfactual_amnt_list %>%
      parallel::mclapply(
        ...
        using_tokens = c(0.10, 0.25, 0.50, 1, 5, 10, 20, 50, 100),
        only_20note = FALSE,
        mc.cores = num_cores,
        mc.preschedule = TRUE,
        mc.cleanup = TRUE
    ...
    save(
      list = c("counter_optimal_tokens_df", "stylized_exercise_3"),
      file = stringr::str_c(
        "extensions-counterfactual-results_", Sys.Date(), ".RData"
      )
    )

  } else {
    counter_path <- stringr::str_c(
      "extensions-counterfactual-results_", last_counter_run, ".RData"
    )
    load(counter_path)
  ...
  baseline_path <- stringr::str_c("baseline-results_", last_base_run, ".RData")
  load(baseline_path)
  ...
  postcounter_df <- counter_optimal_tokens_df %>%
    ...
    dplyr::inner_join(
      ...
      y = counter_optimal_tokens_df %>%
        dplyr::bind_cols(., bind_rows(tran_df, tran_df)) %>%
...
#==============================================================================#
        # COUNTERFACTUAL SIMULATION: ELIMINATION OF THE PENNY & NICKEL #
#==============================================================================#
...
if (initial_run == TRUE) {
  ...
  ext_symmetric_pol_df <- counterfactual_sim(rounding_policy = "symmetric")
  ext_asymmetric_pol_df <- counterfactual_sim(rounding_policy = "asymmetric")

  save(
    list = stringr::str_c("ext_", c("asymmetric", "symmetric"), "_pol_df"),
    file = stringr::str_c("extension-policy-results_", Sys.Date(), ".RData")
  )

} else {
  load(
    file = stringr::str_c(
      "extensions-policy-results_", last_counter_run, ".RData"
    )
  )
